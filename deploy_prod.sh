...
# 修改APP_NAME为云效上的应用名
APP_NAME=f-api-gateway

export SPRING_PROFILES_ACTIVE=prod
PROG_NAME=$0
ACTION=$1
CUR_IP=`ifconfig | egrep '172.30|172.18' | awk '{print$2}'`
...
start_application() {
    ...
    nohup java -DDUBBO_IP_TO_REGISTRY="${CUR_IP}" -jar ${JAR_NAME} > ${JAVA_OUT} 2>&1 &
    echo "started java process"
}

stop_application() {
   checkjavapid=`ps -ef | grep java | grep ${APP_NAME} | grep -v grep |grep -v 'deploy_prod.sh'| awk '{print$2}'`
